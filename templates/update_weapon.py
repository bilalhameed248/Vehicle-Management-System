...
class UpdateWeapon(QWidget):
    ...
    def initUI(self):
        ...
        def add_basic_section(title, row, col):
            ...
            self.Wpn_No_input.setText(self.data["Wpn No"])
        ...
        def L_Bty_section(title, row, col):
            ...
            self.L_Bty_Bty_Voltage.setCurrentIndex(combo_index(self.L_Bty_Bty_Voltage, "Bty Voltage"))
        ...
        def Doc_section(title, row, col):
            ...
            self.Doc_Bty_charging_record.setCurrentIndex(combo_index(self.Doc_Bty_charging_record, "Bty charging record"))
            self.Doc_Storage_temp_Humidity_record.setCurrentIndex(combo_index(self.Doc_Storage_temp_Humidity_record, "Storage temp & Humidity record"))
            self.Doc_Firing_record_check.setCurrentIndex(combo_index(self.Doc_Firing_record_check, "Firing record check"))
            self.Doc_Svc_ability_Completeness_of_tools_accy.setCurrentIndex(combo_index(self.Doc_Svc_ability_Completeness_of_tools_accy, "Svc ability & Completeness of tools & accy"))
            self.Doc_Self_test_record_check.setCurrentIndex(combo_index(self.Doc_Self_test_record_check, "Self test record check"))
            self.Doc_Is_eARMS_fully_func.setCurrentIndex(combo_index(self.Doc_Is_eARMS_fully_func, "Is eARMS fully func and all the processes involved are being carried out through eARMS"))
            self.Doc_Complete_eqpt_inventory_update_on_eARMS.setCurrentIndex(combo_index(self.Doc_Complete_eqpt_inventory_update_on_eARMS, "Complete eqpt inventory update on eARMS"))
            self.Doc_DRWO_work_order_being_processed_on_eARMS.setCurrentIndex(combo_index(self.Doc_DRWO_work_order_being_processed_on_eARMS, "DRWO/ work order being processed on eARMS"))
            self.Doc_Are_Log_book_maintain_properly.setCurrentIndex(combo_index(self.Doc_Are_Log_book_maintain_properly, "Are Log book maintain properly"))
        ...
        def Status_section(title, row, col):
            ...
            self.Status.setCurrentIndex(combo_index(self.Status, "Status"))
    ...
    def save_weapon(self):
        ...
        is_data_inserted = self.db_obj.update_weapon(add_weapon_data, self.data['id'])
        if not is_data_inserted:
            QMessageBox.warning(self, "Failed", "Error while updating the data..! Please Try Again")
            return
        else:
            QMessageBox.information(self, "Success", "Weapon Updated successfully!")
            self.cancel_button()
        
    def cancel_button(self):
        if hasattr(self.main_parent_welcome, "all_weapon_obj"):
            self.main_parent_welcome.all_weapon_obj.populate_table()

            # Switch back to ViewALLVehicles
        self.main_parent_welcome.content_area.setCurrentWidget(self.main_parent_welcome.all_weapon_obj)
        ...
        self.main_parent_welcome.update_weapon_obj = None